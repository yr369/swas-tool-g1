...
# ---------------------------------------------------------------------
# (payload, expected-evaluated-result) pairs across the common template
# engines. Each payload is a harmless arithmetic expression - if the
# *evaluated* result comes back in the response, the engine executed our
# input as code, which is the proof bar (not just "no error thrown").
_SSTI_PROBES: list[tuple[str, str]] = [
    ("${7*7}", "49"),          # Java (Freemarker/Velocity/Thymeleaf-ish)
    ("#{7*7}", "49"),          # Java EL / Ruby-style
    ("{{7*7}}", "49"),         # Jinja2/Twig/Django
    ("{{=7*7}}", "49"),        # Some PHP template engines
    ("<%= 7*7 %>", "49"),      # ERB
]
...
async def check_ssti(url: str) -> dict | None:
    ...
    and checks whether the *evaluated* result (49) shows up in the
    response body somewhere the raw payload didn't already appear. This
    is proof-based, not signature-based - a template engine that just
    echoes "{{7*7}}" back verbatim isn't vulnerable, so echoed-but-
                    ...
                    if expected in body:
                        return {
                            ...
                            "evidence": (
                                ...
                                f"response body (payload itself not present unevaluated), consistent "
                                f"with server-side template injection / RCE."
                            ),
                        }
    except httpx.HTTPError as exc:
