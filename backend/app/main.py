...
async def _finalize_scan_status(pool, project_id: int, tasks: list) -> None:
    ...
    new_status = "error" if failures else "completed"
    async with pool.acquire() as conn:
        await conn.execute(
...
async def lifespan(app: FastAPI):
    ...
    async with pool.acquire() as conn:
        ...
        if stuck_project_rows:
            await conn.execute("UPDATE projects SET status = 'error' WHERE status = 'scanning'")
    ...
    if stuck_project_rows:
        logger.warning(
            ...
            "to 'error' (orphaned by process restart, not a real failure - "
            "safe to re-trigger a scan): %s",
